      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-10">
            {[
              { label: "Total Orders", value: stats.totalOrders, icon: "📦", badge: "All", badgeColor: "bg-yellow-500/10 border-yellow-500/20 text-yellow-800" },
              { label: "Paid Revenue", value: "₹" + (stats.totalSales || 0).toFixed(0), icon: "💰", badge: "Paid Only", badgeColor: "bg-emerald-50 border-emerald-200 text-emerald-800" },
              { label: "Today Orders", value: stats.todayOrders, icon: "📅", badge: "Today", badgeColor: "bg-blue-50 border-blue-200 text-blue-800" },
              { label: "Pending Orders", value: stats.pendingOrders, icon: "⏳", badge: "Pending", badgeColor: "bg-orange-50 border-orange-200 text-orange-800" },
              { label: "Paid Orders", value: stats.paidOrders, icon: "✅", badge: "Completed", badgeColor: "bg-emerald-50 border-emerald-200 text-emerald-800" },
            ].map((stat, i) => (
              <div key={i} className="relative group p-6 rounded-3xl bg-white border border-yellow-500/12 hover:border-yellow-500/35 transition-all duration-300 hover:-translate-y-1 shadow-card hover:shadow-card-hover overflow-hidden">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className={"text-[10px] font-bold px-2.5 py-1 rounded-full border font-grotesk uppercase tracking-wider " + stat.badgeColor}>
                    {stat.badge}
                  </span>
                </div>
                ...
                <div className="flex gap-1.5 p-1 bg-[#F5F2EB] rounded-xl border border-yellow-500/15 w-full sm:w-auto">
                  {["Daily", "Monthly"].map((p) => (
                    <button
                      ...
                      className={"flex-1 sm:flex-none px-4 py-1.5 rounded-lg text-xs font-bold font-grotesk uppercase tracking-wider transition-all " + (period === p.toLowerCase() ? "bg-gradient-to-r from-yellow-500 to-amber-600 text-black shadow-xs" : "text-[#6C685F] hover:text-[#1C1A16]")}
                    >
                      {p}
                    </button>
                  ))}
